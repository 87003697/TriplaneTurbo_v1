...
@threestudio.register("multiprompt-defered-renderer-multistep-generator-scene-system-v1")
class MultipromptDeferedRendererMultiStepGeneratorSceneSystemV1(BaseLift3DSystem):
    @dataclass
    ...
    def forward_rendering(
        self,
        batch: Dict[str, Any],
        phase: str = "1st", # 1st or 2nd
    ):

        # Use different renderers based on phase
        render_out = self.renderer(**batch, phase = phase)

        # Decode RGB latents only during testing and validation
        if self.cfg.rgb_as_latents and not self.training:
            if "comp_rgb" in render_out:
    ...
    def compute_guidance_n_loss(
        self,
        render_output: Dict[str, Any], # Renamed 'out' to 'render_output' and removed unused out_2nd
        idx: int,
        **batch,
    ) -> Dict[str, Any]:
        # guidance for the first renderer
        guidance_rgb = render_output["comp_rgb"]
        ...
        guidance_out = self.guidance(
            guidance_rgb, 
            normal=render_output.get("comp_normal_cam_vis"), # Use .get() for safety
            depth=render_output.get("disparity"), # Use .get() for safety
            **batch, 
            rgb_as_latents=self.cfg.rgb_as_latents,
            timestep_range=timestep_range,
        )

        # Determine if this is output from 1st or 2nd phase renderer
        renderer_type_for_loss = "1st" # Default
        if "phase2_complete" in render_output and render_output["phase2_complete"]:
            renderer_type_for_loss = "2nd"
        elif "phase1_complete" in render_output and render_output["phase1_complete"]:
            renderer_type_for_loss = "1st"
        # Add any other conditions if necessary, or rely on the default if no flags are present

        loss_dict = self._compute_loss(
            guidance_out, 
            render_output, 
            renderer=renderer_type_for_loss, # Dynamically set
            step = idx, 
    ...
    def _training_step_rollout_rendering_distillation(
        ...
        for i, (t, batch) in enumerate(zip(timesteps, batch_list)):
            ...
            if only_last_step and i < len(timesteps) - 1:
                ...
                out = self.forward_rendering(batch, phase = "1st")
                ...
                self.manual_backward(loss_render, retain_graph=True)

                out_2nd = self.forward_rendering(batch, phase = "2nd")
                loss_dict_2nd = self.compute_guidance_n_loss(
                    out_2nd, idx = i, **batch
                )
                fidelity_loss_2nd = loss_dict_2nd["fidelity_loss"]
                regularization_loss_2nd = loss_dict_2nd["regularization_loss"]

                loss_render_2nd = (
                    fidelity_loss_2nd + regularization_loss_2nd
                )  / self.cfg.gradient_accumulation_steps / (1 if only_last_step else self.cfg.num_steps_training)

                self.manual_backward(loss_render_2nd)
    ...
    def test_step(self, batch, batch_idx_loop, return_space_cache = False, render_images = True): # Renamed batch_idx
