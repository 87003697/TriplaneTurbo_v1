...
@threestudio.register("generative-space-3dgs-rasterize-renderer-v4")
class GenerativeSpace3dgsRasterizeRendererV4(Rasterizer):
    @dataclass
    ...
    def _forward(
        ...
        **kwargs
        ...
        # the center point depth map, achieved by setting scale to 0
        _, _, center_point_depth_map, center_point_opacity_map  = rasterizer(
            means3D=pc.xyz,
            means2D=torch.zeros_like(pc.xyz, dtype=torch.float32, device=pc.xyz.device), # Use zero means2D if not using screenspace_points
            shs=None,
            colors_precomp=pc.rgb,
            opacities=torch.ones_like(pc.opacity),
            scales=torch.zeros_like(pc.scale),
            rotations=pc.rotation,
            cov3D_precomp=None,
        )
        ...
        return {
            ...
            "center_point_depth": center_point_depth_map, # Use the initialized/updated map
            "center_point_opacity": center_point_opacity_map, # Use the initialized/updated map
        }
    ...
    def forward(
        ...
        **kwargs # Catch unused arguments
        ...
        outputs = {
            ...
            "comp_center_point_opacity": comp_center_point_opacity, # ADDED
        }
