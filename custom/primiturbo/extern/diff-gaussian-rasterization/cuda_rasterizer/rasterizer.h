...
namespace CudaRasterizer
{
	...
	public:
		...
		static void compute_center_depth(
			...
			unsigned char* out_opacity,  // Output opacity map (H, W) - Using unsigned char for byte
			float* out_depth,            // Output depth map (H, W)
			bool debug                   // Debug flag
			// Note: Buffers (geom, binning) omitted for now, assuming simpler kernel.
			// May need to add std::function<char*(size_t)> later if sorting/tiling is required.
		);
	};
